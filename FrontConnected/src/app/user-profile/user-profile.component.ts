...
export class UserProfileComponent implements OnInit {
	...
	@Output()
	accountDeletedEvent = new EventEmitter();
	...
	ngOnInit(): void {
	  if (this.authService.isAuthenticated()) {
		this.searchService.searchedUser.subscribe(u => {
		  ...
		  this.seachedUser = u;
		  this.seachedUser.retrievedImage = "data:image/png;base64," + u.picByte;
		  //console.log(this.seachedUser.retrievedImage);
	...
	getImage(){
	  this.http.get('http://localhost:8080/api/image/' + this.user.userId)
	  .subscribe(
		res => {
	...
	myFunction(e) {
	  ...
	  deleteAccount(){
		this.authService.delete(this.user.userId).subscribe(
			(produs) => {
			  this.accountDeletedEvent.emit();
